...
export default function ContratoPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // ✅ Si viene token, buscamos por token (lo ideal para cliente)
      if (token) {
      ...
      // ✅ Si no hay token, usamos id (admin / interno)
      if (!id) {
        setLoading(false);
        return;
      }
  ...
  const textoContrato = useMemo(() => {
    ...
    return [
      ...
      "",
      `Por medio del presente, ${cliente.nombre_completo} solicita un préstamo por la cantidad de ${money(
        prestamo.monto
      )}, a pagarse en ${prestamo.quincenas} quincenas.`,
      "",
      `El pago quincenal será de ${money(prestamo.pago_quincenal)}, con un total a pagar de ${money(
        prestamo.total_a_pagar
      )}.`,
      "",
      `La fecha de inicio es ${formatFechaMX(prestamo.fecha_inicio)} y la fecha de término es ${formatFechaMX(
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-3xl mx-auto bg-white rounded-xl shadow p-6">
          ...
          <p className="mt-2 text-red-600">Falta <b>?token=</b> (cliente) o <b>?id=</b> (interno).</p>
        </div>
      </div>
    );
  }
